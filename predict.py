...
test_labels = test.loc[:, 'area':'vertical_interval']
test_image_id = test['image_id'].tolist()
# test_labels.reset_index(drop=True, inplace=True)
# test.image_id.reset_index(drop=True, inplace=True)

IMAGE_FOLDER = osp.join(args.root,'images/')

def get_image_path(filename):
    # import ipdb;ipdb.set_trace()  
    # return (IMAGE_FOLDER + filename + '.jpg')
    return (IMAGE_FOLDER +  type_list[test_labels.loc[test_image_id.index(filename)].tolist().index(1)]+ '/' + filename + '.jpg')

# test = pd.read_csv(osp.join(args.root,'test.csv'))
test['image_path'] = test['image_id'].apply(get_image_path)
test_paths = test.image_path


# model_name = 'resnest200'
# model = resnest200(model_path=None)
...
# model = create_model(args.model,model,Path(f'./pl_ckpts/model_efficientnet-b5_albu/best-model.pt'))
# model = create_model(args.model,model,Path(f'./pl_ckpts/model_efficientnet-b5_albu_re/best-model.pt'))
# model = create_model(args.model,model,Path(f'./pl_ckpts/model_efficientnet-b5_albu_mixup_re/best-model.pt'))
# model = create_model(args.model,model,Path(f'./pl_ckpts/model_efficientnet-b5_albu_mixup_lbs_fixed/best-model.pt'))
# model = create_model(args.model,model,Path(f'./pl_ckpts/model_efficientnet-b5_albu_re_cj/best-model.pt'))
# model = create_model(args.model,model,Path(f'./pl_ckpts/model_efficientnet-b7_albu_re/best-model.pt'))
model = create_model(args.model,model,Path(args.ckpt_file))
...
output['image_path'] = test.image_path
